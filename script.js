...
async function call(payload) {
    ...
    if (!text.trim()) return null; // Retorna null se a resposta for vazia, sem gerar erro
    try {
...
async function iniciar() {
    ...
    // Carrega dados base
    try {
        const resp = await call({ action: "obterDelegacias" });
        if (Array.isArray(resp)) {
            listaDelegacias = resp;
        } else {
            throw new Error("Servidor não retornou lista válida.");
        }
    } catch (e) {
        console.error("Erro ao carregar lista de delegacias:", e);
        return Swal.fire({
            icon: 'error',
            title: 'Erro de Comunicação',
            text: 'Não foi possível carregar as unidades policiais do servidor. Verifique sua conexão e recarregue a página.',
    ...
    inputDelegacia.addEventListener('change', function () {
        if (this.value && !listaDelegacias.includes(this.value)) {
...
function validarERevisar() {
    ...
    if (!unidade || !listaDelegacias.includes(unidade)) {
        inputUnidade.focus();
...
function finalizarSessaoSemRecarregar(idSucesso) {
    ...
    // Desabilita os botões de rascunho após finalizar
